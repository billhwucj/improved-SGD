...
def load_scene():
    ...
    try:
        with open('splats_rooms.json', 'r') as file: # change to the json file name that you want to load
    ...
    def generate_batches(batch_size, gaussian_count):
        ...
        for batch_index in range(num_batches):
            ...
            # ⭐ NEW: compute batch size in bytes
            batch_json = json.dumps(data)
            batch_bytes = len(batch_json.encode("utf-8"))

            # ⭐ NEW: compute delay to simulate 9.82 MB/s
            delay = batch_bytes / TARGET_BW
            time.sleep(delay)
